...
version: v1.0
Author: cheng jie 
Date: 2021-03-15 09:57:19
LastEditors: Please set LastEditors
LastEditTime: 2021-08-11 07:10:40
'''
...
class Dataloader(tf.keras.utils.Sequence):
    ...
    def data_generate(self, record_list):
        ...
        if self.model == 'train'  or self.model == 'eval': 
            return [batch_crop_images, batch_screen_images, batch_head_image, batch_anno], batch_anno
        elif self.model == 'test':
            return batch_path, [batch_crop_images, batch_screen_images, batch_head_image], batch_anno
    ...
    def _parse_list(self):
        self.video_list = [VideoRecord(x.strip().split(',')) for x in open(self.data_list)]
...
#     data_loader = Dataloader(batch_size=16, data_list='/mnt/sda2/cj/headscreen_cls/data_list_20210222/test_list_new.txt', num_class=2, crop_h=224, crop_w=224,
#                     head_img_h=96, head_img_w=96, screen_img_h=96, screen_img_w=96, shuffle=True)
    
#     for step,(x_train_batch, label_train_batch) in enumerate(data_loader):
#         print(step, x_train_batch[0].shape, x_train_batch[1].shape, x_train_batch[2].shape, x_train_batch[3].shape, label_train_batch.shape)
